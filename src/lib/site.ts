export const SITE = {
  name: "Escritório Gonçalves",
  legal: "Gonçalves Sociedade Individual de Advocacia",
  cnpj: "49.235.654/0001-93",
  oab: "OAB/RO 12.345", // Ajustar com a inscrição real
  phone: "(69) 9.9262-1298",
  email: "contato@escritoriogoncalves.com",
  whatsapp:
    "https://api.whatsapp.com/send?phone=5569992621298&text=Olá,%20Renan!%20Gostaria%20de%20tirar%20dúvidas%20sobre%20direito%20previdenciário.",
  instagram: "https://www.instagram.com/renan.inss/",
  tiktok: "https://www.tiktok.com/@renan.inss",
  youtube: "https://www.youtube.com/c/DireitoEmMinutos/",
  cities: "Jaru (RO) · Alta Floresta D'Oeste (RO)",
} as const;
